...
开始请创建一个名为links-and-images的新文件夹以存储我们的所有文件。您应该能够在Atom中使用我们在“简介”一章中所遵循的相同过程。
...
请注意，您可以在Atom中创建一个新文件夹，方法是右键单击文件浏览器窗格，然后在上下文菜单中选择新建文件夹。生活是更好的，当你从来没有需要离开你的文本编辑器。

#### 图像下载

我们将在我们的images.html文件中嵌入图像，所以一定要下载这些示例mochi图像。将其解压缩到您的links-and-images文件夹，从ZIP文件保持父图像文件夹。您的项目现在应该如下所示：

![Image](../images/links-and-images/3.png)

### 锚点

链接使用<a>元素创建，表示“anchor”。它的工作方式与上一章中的所有元素相同：当您在<a>标签中包装某些文本时，它会更改该内容的含义。让我们通过在links.html的<body>元素中添加以下段落来看看：

```html
<p>This example is about links and <a>images</a>.</p>
```

如果您在网络浏览器中加载网页，您会注意到<a>元素根本不像链接。是的，不幸的是，<a>元素本身不做任何事情。
...
不同的元素允许不同的属性，您可以参考MDN了解哪些元素接受哪些属性的详细信息。现在，我们关心的是href属性，因为它决定了用户点击<a>元素时所处的位置。更新您的链接以匹配以下内容：
...
现在我们正在使用链接，我们需要了解网站的结构。为了我们的目的，网站只是组织成文件夹的HTML文件的集合。为了从另一个文件内部引用这些文件，因特网使用“统一资源定位符”（URL）。根据您所指的是什么，网址可以采用不同的形式。我们将要处理的三种类型的网址在下面突出显示：

![Image](../images/links-and-images/6.png)

绝对，相对和根相对链接指的是href属性的值。接下来的几节将解释如何和何时使用它们。但首先，我们将以下内容添加到我们的links.html文件：
...
“绝对”链接是您可以引用Web资源的最详细的方式。它们以“scheme”（通常是http：//或https：//）开头，后面是网站的域名，然后是目标网页的路径。
...
“相对”链接指向您正在编辑的文件的有利位置的网站中的另一个文件。这意味着该方案和域名与当前页面相同，因此您需要提供的唯一的东西是路径。
...
当您在网络浏览器中单击这些链接时，它会抱怨该页面不存在。检查地址栏，您会发现浏览器正在尝试加载misc / links.html和misc / images.html，它正在查找错误的文件夹！这是因为我们的链接是相对于extras.html的位置，它居住在misc文件夹中。

![Image](../images/links-and-images/10.png)

要解决这个问题，我们需要..语法。文件路径中的两个连续点表示指向父目录的指针：
...
“根相对”链接与上一部分类似，但它们不是相对于当前页面，而是相对于整个网站的“根”。例如，如果您的网站托管在our-site.com上，则所有根相对URL都将相对于our-site.com。
...
根相对链接和相对链接之间的唯一区别是前者以正斜杠开始。该初始正斜杠表示您网站的根。您可以在初始斜杠之后的路径中添加更多文件夹和文件，就像相对链接一样。无论当前页面位于何处，以下路径都可以正常工作（即使在misc / extras.html中）：
...
属性改变了HTML元素的含义，有时您需要修改元素的多个方面。例如，<a>元素还接受定义在用户单击链接时显示页面的目标属性。默认情况下，大多数浏览器用新的页面替换当前页面。我们可以使用target属性让浏览器在新窗口/标签中打开一个链接。
...
在地址栏中，您将看到我们的所有空格都被替换为％20，如上所示。网址中不允许使用空格，这是用于表示它们的特殊编码。而不是一个空格，你应该总是使用连字符，正如我们在整个教程一样。使用所有小写字符来保持一致性也是一个好主意。

请注意，我们的文件/文件夹名称与其所代表的网页的网址之间有直接连接。我们的文件夹和文件的名称确定我们的网页的。。它们对用户可见，这意味着您应该投入尽可能多的努力来命名文件，当您创建它们包含的内容。

这些命名约定适用于您网站中的所有文件，而不仅仅是HTML文件。 CSS文件，JavaScript文件和图像应避免空格，并具有一致的大小写。
...
接下来的几个小节将分别介绍每种图像格式的理想用例。在继续之前，请确保您已将这些mochi图片解压缩到links-and-images项目中。
...
GIF是简单动画的首选选项，但是折中的是它们在调色板方面有些局限 - 从不使用它们作为照片。透明像素是GIF的二进制选项，意味着您不能有半透明像素。这可能使得难以在透明背景上获得高水平的细节。因此，如果您不需要动画，通常最好使用PNG图像。

![Image](../images/links-and-images/15.gif)

您可以使用以下内容将这个小家伙添加到我们的images.html文件中：
...
SVG有一个潜在的问题：为了让它们跨浏览器一致地显示，您需要使用图像编辑器（例如，Abode Illustrator或Sketch）将任何文本字段转换为大纲。如果您的图片包含大量文本（例如本教程中的幻灯片截图），这可能会对文件大小产生很大影响。因此，InternetingIsHard.com使用PNG而不是SVG，即使SVG是如此真棒。
...
width和height属性可能很有用，但通常最好使用CSS设置图片尺寸，以便您可以使用媒体查询进行更改。我们将到[响应式设计]()更详细地讨论这一点。

### 替代文本

向您的<img />元素添加alt属性是最佳做法。它定义了正在显示的图像的“文本替代”。这对具有纯文本浏览器的搜索引擎和用户（例如，由于视觉障碍而使用文本到语音软件的人）具有影响。
...
现在，我们（希望）更加乐于使用HTML属性语法，我们可以为HTML样板代码添加几个整理。您创建的每个网页都应该定义它所写的语言及其字符集。
...
如果您不确定您的语言的国家/地区代码是什么，您可以在[这里](http://www.iana.org/assignments/language-subtag-registry/language-subtag-registry)查看它在子标签字段。
...
特殊字符现在应正确呈现。现在，UTF-8类似于互联网的通用字母表。您创建的每个网页都应在其<head>中包含此行。

### html实体

好吧，所以这最后一节实际上没有任何关系到链接或图像，但我们需要讨论一个事情之前切换齿轮到CSS。 “HTML实体”是一个特殊字符，不能在HTML文档中表示为纯文本。这通常意味着它是HTML中的保留字符，或者您的键盘上没有键。
...
卷曲的报价是绝对不必要的，但如果你关心的排版，他们会是一些 您将使用的最常见的HTML实体。有四种不同的卷曲报价（开始和结束单引号和双引号）：
...
我们还学习了一些重要的属性（lang和charset），它们为我们提供了基本的模板，您应该使用它作为每个网页的开始：
